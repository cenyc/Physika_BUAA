...
#define NUM_CLOUD_FILES		30		//number of cloud data files
...
VolumetricClouds::VolumetricClouds()
{
	Albedo = 0.95f;
	Extinction = 85.0f;
	SplatBufferSize =32;		//size of the splat buffer (this is a good value)ʲô���ã�������������
	ImpostorSize = 512;			//maximum size of the impostor
}

void VolumetricClouds::GrowCloud(VolumetricCloud* Cloud, int level, float radius, Vector3 Position)//ͨ�����ļ�������
{
	FILE* fp = NULL;
	char strname[256];
#ifdef WIN32	
	...
	//sprintf(strname, "clouds\\cloud1.dat");
	//sprintf(strname,"D:\\yuancodes\\GenParticle\\GenParticle\\particles\\cloud0.dat");
	//sprintf(strname,"bak\\cloud5.dat");
	sprintf(strname,"bak\\cloud%d.dat", rand() % 10);
#else
//	sprintf(strname, "clouds/cloud%d.dat", rand() % NUM_CLOUD_FILES);
//    sprintf(strname,"clouds/cloud0.dat");
    //sprintf(strname,"clouds/cloudmesh2-my.dat");
//    sprintf(strname,"/home/vr715/project/CloudModelRender/cudatest/test/cloud0.dat");
	strcpy(strname,(get_cwd()+"/../../data/Cloud_Render/Harris/clouds/cloud1.dat").c_str());
    //sprintf(strname, "/home/czn/Desktop/CloudRendering/clouds/cloud1.dat");
    //sprintf(strname,"clouds/mycloud.dat");
#endif
	...
	for (int i = 0; i < Num; i++)
	{
		...
		/*if(i<=300)		//zzl���ԣ�����λ��ƽ��
			puff.Position += (Position+Vector3(i,i,i));
		else*/
...
void VolumetricClouds::GenerateTexture()
{
	...
	for (int y = 0; y < N; y++)
	{
		...
		for (int x=0; x<N; x++, i++, j+=4)
		{
			...
			B[j+3] = B[j+2] = B[j+1] = B[j] = (unsigned char)(value * 255);//Ӱ�����ӿ����Ĵ�С
...
int VolumetricClouds::Create(int Num, float PlaneSize, float PlaneHeight)
{
	...
	for (i = 0; i < Num; i++)//Num��ʾҪ�������ٶ���
	{
		VolumetricCloud Cloud;
		//Cloud.Center.x = rand() % (int)(PlaneSize * 2) - PlaneSize;//��֤ԭ��������λ��
...
void VolumetricClouds::Update(Vector3 Sun, Vector3 Camera)
{
	...
	for (unsigned i = 0; i < Clouds.size(); i++)
	{
		...
		if (Dot(SunDir, Clouds[i].LastLight) < 0.99f)//????Ϊʲô��������Ƕ�
		{
...
void VolumetricClouds::Render(Vector3 Camera, Vector3 Sun)//�����������Ⱦ
{
	unsigned i, j;
	...
	for (i = 0; i < Clouds.size(); i++)
	{
		dist_impostor = Clouds[i].Radius * DIST_MUL_IMPOSTOR; //beyond this render only the impostor��Ϊ�涨�ľ���
		...
		if (!Frustum.SphereInFrustum(Clouds[i].Center, Clouds[i].Radius)) continue;//������׶����������ѭ������ִ��

		if (Clouds[i].DistanceFromCamera > dist_impostor)
			//fully impostor
			RenderCloudImpostor(&Clouds[i], 1.0f);//Դ���룬û�оͻ���ֶ�������
			
		else
			if (Clouds[i].DistanceFromCamera < dist_3D)
				//fully 3D
				RenderCloud3D(&Clouds[i], Camera, Sun, 1.0f);
			else//ͼ������Ҫ3d��ʾ����Ҫimposeter��ʾ֮��
			{
				...
				RenderCloudImpostor(&Clouds[i], ALPHA_ADJUST + alpha);//û�оͻ���ֶ�������
				RenderCloud3D(&Clouds[i], Camera, Sun, 1.0f - alpha);
			}
	}
}

void VolumetricClouds::MakeCloudImpostor(VolumetricCloud* Cloud, Vector3 Sun, Vector3 Camera)//���ƴ洢������Ϊimpostorʹ��
{
	unsigned i;
	...
	ViewportSize = GetImpostorSize(d * d);//���ݾ������imposter�Ĵ�С
	...
	if (glIsTexture(Cloud->ImpostorTex) == GL_FALSE)//�ж��Ƿ��ǵ�һ����Ⱦ
...
void VolumetricClouds::LightCloud(VolumetricCloud* Cloud, Vector3 Sun)//�ƵĹ��ռ���
{
//	assume Sun is a point!!
	Sun = Normalize(Sun) * Cloud->Radius * 1.5f + Cloud->Center;//��̫������Զ��
...
//	puffs are now in world space
	...
	float d = Dist(Sun, Cloud->Center);//̫���������ľ���
	float r = Cloud->Radius;//�Ƶİ뾶
	float pr = Cloud->Puffs[0].Size;//��һ�����Ӵ�С
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(-Cloud->Radius-pr, Cloud->Radius+pr, -Cloud->Radius-pr, Cloud->Radius+pr, d - r, d + r);//Դ����
	//glOrtho(-1, 1, -1, 1, d - r, d + r);//zzl����ͶӰλ��	
	//setup the camera to lookat the cloud center and be positioned on the sun
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glLoadIdentity();		
	gluLookAt(Sun.x, Sun.y, Sun.z,Cloud->Center.x, Cloud->Center.y, Cloud->Center.z, 0, 1, 0);//̫��λ��Ϊ�ӵ�λ��
	...
	glAlphaFunc(GL_GREATER, 0);//alpha����
	
	glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);//��ͼģʽ
	...
	{		
		...
		Area = Cloud->Puffs[i].DistanceToCam * SolidAngle;//���������solidangleΪ�����
		Pixels = (int)(sqrt(Area) * SplatBufferSize / (2 * Cloud->Radius));//??????�����ظ���
		...
		glReadBuffer(GL_BACK);//˫��������Ĭ����ʽ
		glReadPixels(ReadX, ReadY, Pixels, Pixels, GL_RED, GL_FLOAT, buf);//��r��Χ�ڵ���������
		...
		ParticleColor.R = LightColor.R * Albedo * Extinction * avg * factor;//����ÿ�����ӵĹ����ɫ
		...
		Cloud->Puffs[i].Color.Clamp();//��ɫֵ��Խ����ʹ���
...
void VolumetricClouds::RenderCloud3D(VolumetricCloud* Cloud, Vector3 Camera, Vector3 Sun, float alpha)
{
	...
	glEnable(GL_TEXTURE_2D);//��������ͻ�Ϲ���
	glBindTexture(GL_TEXTURE_2D, PuffTexture);
	
	glEnable(GL_BLEND);
	glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
	
	Vector3 Light = Normalize(Camera - Sun);//ԭʼ�����۾���̫�����������
	...
	Vector3 Corner1, Corner2, Corner3, Corner4;
	Corner1 = -vx - vy; Corner2 = vx - vy; Corner3 = vx + vy; Corner4 = vy - vx;//4���ǵ���չ
	
	for (PuffIter = Cloud->Puffs.begin(); PuffIter != Cloud->Puffs.end(); ++PuffIter)
	{		
		Puff = &*PuffIter;
		//Vector3 Light=Normalize(Puff->Position-Sun);//zzl�޸ģ�̫�������ӵķ���
		Omega = Puff->Position - Camera;
		
		omega2 = Omega.x * Omega.x + Omega.y * Omega.y + Omega.z * Omega.z;
		omega2 = carmack_func(omega2);//��omega2����
		
		//omega2 is now 1 / Mag(Omega)
		Omega.x *= omega2;	Omega.y *= omega2;	Omega.z *= omega2;//�۾������ӵķ�������
		
		costheta = Dot(Omega, Light);	
		phase = 0.75f * (1.0f + costheta * costheta);//p(Q)=3/4*(1+cosQ^2)�ຯ��
	
		ParticleColor.R = ( 0.3f +Puff->Color.R * phase) * alpha;//0.3����������ã������ƺ�û������Ч����Ӱ�죿����
		ParticleColor.G = ( 0.3f +Puff->Color.G * phase) * alpha;//����ȫ3Dʱ��alpha=1
		ParticleColor.B = (0.3f + Puff->Color.B * phase) * alpha;
		ParticleColor.A = Puff->Color.A * Puff->Life * alpha;
				
		*(vp++) = Puff->Position + Corner1 *Puff->Size;//�����ӿ���һ�����Σ�����ĸ��ǵ�����
		*(tp++) = v1;
		*(cp++) = ParticleColor;
		...
		*(vp++) = Puff->Position + Corner4 *Puff->Size ;//Puff->Size
		*(tp++) = v4;
		*(cp++) = ParticleColor;
	}
	...
	//GLfloat light_position[]={0.0,0.0,1.0};//zzl
	//GLfloat white_light[]={1.0,1.0,1.0,1.0};//zzl
	//glLightfv(GL_LIGHT0,GL_POSITION,light_position);//zzl
	//glLightfv(GL_LIGHT0,GL_DIFFUSE,white_light);
	//glLightfv(GL_LIGHT0,GL_SPECULAR,white_light);
	//glEnable(GL_LIGHTING);//zzl
	//glEnable(GL_LIGHT0);//zzl
