...
int    GLUI_Rotation::iaction_mouse_down_handler( int local_x, int local_y )
{
  ...
  local_y = (int) floor(2.0f * ball->center[1] - local_y);
...
void    GLUI_Rotation::iaction_draw_active_area_ortho( void )
{
  ...
  for( k=0; k<60; k++ ) {
    float phi = TWOPI * k / 60.0f;
    vec2 p( cos(phi) * (2.0f + radius), sin(phi) * (2.0f + radius));
  ...
  for( k=0; k<60; k++ ) {
    float phi = TWOPI * k / 60.0f;
    vec2 p( cos(phi) * (1.0f + radius), sin(phi) * (1.0f + radius));
    if ( enabled ) {
