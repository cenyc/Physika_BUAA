...
void Cloud::CreateCloudMesh(Image image, Pixel pixel, Sky sky, Mesh &mesh, float* heightField){
	...
	{
		mesh.Cloud_vertexList[3 * i + 0] = mesh.vertexList[3 * i + 0];
		mesh.Cloud_vertexList[3 * i + 1] = mesh.vertexList[3 * i + 1];
		mesh.Cloud_vertexList[3 * i + 2] = InterPolateHeightField(image, pixel, sky, mesh.vertexList[3 * i + 0], mesh.vertexList[3 * i + 1], heightField);
